...
- Playback is a state machine (`src/lib/scene-playback.ts`) with one Pause / Resume toggle, separate Replay, reduced-motion / save-data static poster, hidden / off-screen suspend, and no auto-resume after a user pause.
...
- Night is a grade plus layered, softly blurred SVG aurora curtains and subtle water reflections on the locked fjord. There is no WebM or MP4 in this tip.
